...
export default class ZoomableImageView extends ImageView {
  ...
  constructor(params?: Partial<ImageView> & IZoomable) {
    ...
    this.__ios = {};
    if (params?.animated) {
      this.__animated = params.animated;
    }
    if (params?.zoomScale) {
      this.zoomScale = params.zoomScale;
    }
    ...
    //@ts-ignore
    this.imageView.nativeObject.imageDidSet = () =>
  ...
  get zoomScale(): number {
    return this.scrollView.zoomScale;
  }

  set zoomScale(value: number) {
    if (value > this.maximumZoomScale) {
      this.scrollView.setZoomScaleAnimated(
      ...
      this.scrollView.setZoomScaleAnimated(value, this.animated);
    }
  }
  ...
  calculateImageViewFrame = (frame?: any) => {
    ...
    if (
      this.imageFillType === ImageView.FillType.ASPECTFILL ||
      this.imageFillType === ImageView.FillType.STRETCH ||
      this.imageFillType === ImageView.FillType.ASPECTFIT
    ) {
      ...
      // @ts-ignore
      this.imageView.image.nativeObject &&
      //@ts-ignore
      this.imageView.image.nativeObject.size
    ) {
      let innerFrame = frame || this.scrollView.frame;
      let image = this.imageView.image as Image;
      let width =
        image.width < innerFrame.width ? innerFrame.width : image.width;
      let height =
        image.height < innerFrame.height ? innerFrame.height : image.height;
      //@ts-ignore
      this.imageView.nativeObject.frame = {
        x: 0,
        y: 0,
        width: width,
        height: height,
